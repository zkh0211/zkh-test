...
import '../analysisForPolice/analysisForPolice.less'
...
import {Button, Carousel, Col, Modal, Row, Table , message } from "antd";
...
export default class PcszhzFankui extends Component{
    ...
    getTaskWillTodo(){
        // console.log(1234566)
        this.props.dispatch(pcszhzDaiban({urlParam:this.state.id},(res)=>{
            if(res.data.length>0){
                this.setState({
                    taskWillTodo:res.data,
                    feedCurrentPushId:res.data[0].submitId
                })
            }else{
                this.setState({
                    taskWillTodo:[]
    ...
    goAfter(current){
        // console.log(current)
        let currentPushId = this.state.taskWillTodo.length>0?this.state.taskWillTodo[current].submitId:''
        this.setState({
    ...
    feedBackText(){
        ...
        if(this.state.feedCurrentPushId!==''){
            // if(false){
            this.props.dispatch(pcsrecvFeedBack(data,(res)=>{
                if(res.status === 200){
                                            ...
                                            this.state.taskWillTodo.length>0?(this.state.taskWillTodo.map((item,index)=>{
                                                return(
                                                    <div className="analysisPolice-willTask" key={index+'11s'}>
                                                        <p>{index+1}</p>
                                                        <p>({item.submitTime?item.submitTime:''})</p>
                                                        <p>{item.taskDesc?item.taskDesc:''}</p>
                                                        <div className="daiban-file">
                                                            {
                                                                item.assignFiles.length>0?(item.assignFiles.map((elem,num)=>{
