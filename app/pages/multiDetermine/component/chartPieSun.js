...
export default class ChartPieSun extends Component {
    ...
    initOption(newProps) {
        ...
        this.option.graphic.style.width=this.option.graphic.style.height=echartWidth*0.27;
        ...
        if (newProps.data.length !== 0) {
            for (var i = 0; i < newProps.data.length; i++) {
                this.option.legend.data[i] = newProps.data[i]['placeTypeName'];
                let x = i % (this.option.color2.length);
                this.option.series[0].data[i] = this.option.series[1].data[i] = {
                    name: newProps.data[i]['placeTypeName'],
                ...
                sum += parseInt(newProps.data[i]['count']);
            }
        }else {
            this.option.series[0].data = this.option.series[1].data =[]
        }
        this.option.title[0].text = '接报警总数';
