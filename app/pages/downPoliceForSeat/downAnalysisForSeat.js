...
import './downAnalysisForSeat.less'
...
import { Radio,Carousel,message , Modal, Button} from 'antd';
moment.locale('zh-cn');
...
export default class DownAnalysisForSeat extends Component{
    constructor(props){
        super(props)
        this.state={
    ...
    getZhipaiXiaji(){
        this.props.dispatch(selectAssignList({urlParam:this.state.id},(res)=>{
            if(res.data.length>0){
                this.setState({
                    xiajiPoliceTypes:res.data
                },()=>{
    ...
    getxiajiPoliceType(){
        this.props.dispatch(getxiajiPoliceTypeList({deptNo:Cookies.get('gxjgdm'),id:this.state.caseInfo.id},(res)=>{
            if(res.data.length>0){
                this.setState({
                    xiajiPolices:res.data
                },()=>{
    ...
    xiajiPoliceOnChange(value) {
        ...
        if(isChecked){
            ...
            this.setState({
                ...
                // console.log('checked')
                // console.log(this.state.deptNos)
            })
        }else{
    ...
    feedBackFile(e){
      ...
      appointDone() {
        //警种编号
        let policeTypeIdList = [];
        let zuoxiTypeIds = this.state.zuoxiTypeId;
        let policeTypeId = this.state.policeTypeId;
        let depts = this.state.deptNos;
        policeTypeIdList = policeTypeIdList.concat(zuoxiTypeIds,policeTypeId);
        let values = {
            "id": this.state.caseInfo['id'],
            "result": 1,
            "secretLevel":this.state.caseInfo.secretLevel,
            "polices": policeTypeIdList,
            "taskDesc": this.refs.taskDesc.value,
            "deptNos": this.state.deptNos,
    ...
    makeReport(){
        ...
        this.props.dispatch(isGenerateReport({urlParam:this.state.id},(res)=>{
            if(res.data){
    ...
    xiajirenwu =(e)=>{
        this.props.dispatch(selectAssign({urlParam:this.state.assignPoliceTypeId},(res)=>{
            if(res.data.length>0){
                this.setState({
                    xiajiTaskWillTodo:res.data,
                    // feedCurrentPushId:res.data[0].pushId
                })
            }else{
                this.setState({
    ...
    showXiajiTasked = (e)=>{
        // console.log('kuaiClick')
        e.stopPropagation();
        let policeType = e.currentTarget.getAttribute('data-submitid');
        this.setState({
            assignPoliceTypeId:policeType
        },()=>{
           this.xiajirenwu();
    ...
    deleteZhipaiTask(e){
        let submitId = e.currentTarget.getAttribute('data-submitid');
        this.props.dispatch(revokeAssign({urlParam: submitId},(res)=>{
            if(res.rel){
                // console.log('revoke')
                this.xiajirenwu();
                this.getZhipaiXiaji()
            ...
            <SuperSearch show={this.state.showSearch} close={this.closeSuperSearch.bind(this)} getData={this.getSuperSearchData.bind(this)}/>
                ...
                <div className="trace-content-middle trace-middle-flex" onClick={this.closePoliceTypes.bind(this)}>
                    <div className="zhipai-left">
                        ...
                        <div className='analysis-middle-zhipai' className={this.state.openSendTask==='0'?'analysis-middle-zhipai analysis-zhipai-close':'analysis-middle-zhipai analysis-zhipai-open'} onClick={this.stopPropagation.bind(this)}>
                            ...
                            <div className="analysis-zhipai-select">
                                ...
                                <p className='analysis-zhipai-title'>选择{this.state.jibieLeixing}：</p>
                                <div className="selectPoliceDatas4">
                                    <Carousel {...settings3}>
                                            {
                                                this.state.xiajiPolices.map((item,index)=>{
                                                //  if(index >= (elem-1)*6 && index < elem*6){
                                                    return(
                                                        <div>
                                                            <div key={item.policeTypeId}  className='trace-middle-item analysis-select-elem'>
                            ...
                            <div className=' feedback-queding'>
                                <button className="button" onClick={this.appointDone.bind(this)}>确定</button>
                            </div>
                        </div>
                    </div>
                    <div className="zhipai-right">
                                    ...
                                    if(parseInt(item.parentId)<0){}
                                        ...
                                        return(
                                            <div className='trace-middle-item trace-item-one' key={index}>
                                                <div className="zuoxiTypes"  data-policetypeid={item.policeTypeId} onClick={this.showTasked}>
                                                    <div className="radio-button analysis-zuoxi">
                                                        <img src={Computer}/>
                                                        <p title={item.policeTypeName}>{item.policeTypeName}</p>
                                                        <p>（<span>{item.backedNum?item.backedNum:'0'}</span>/{item.sumNum?item.sumNum:''}）</p>
                                                    </div>
                                                </div>
                                                {/*指派的任务显示*/}
                                                ...
                                                {
                                                    this.state.taskWillTodo.length>0?(this.state.taskWillTodo.map((datas,index)=>{
                                                     
                                                        return(
                                                            <div key={index} className="showTask-item" >
                                                                <div className='showTask-item-time' >{datas.infoDate?datas.infoDate:''}</div>
                                                                <div className="showTask-item-desc" >
                                                                    <img src={this.state.feedCircle} alt="pic"/>
                                                                    <p>{datas.taskDesc?datas.taskDesc:''}</p>
                                                                    <button className={(datas.pushState === '0')?"cancleTaskButton":'display-none'} data-pushid={datas.pushId} onClick={this.cancleTask.bind(this)}>撤销</button>
                                                                    {
                                                                        datas.files.length>0?(datas.files.map((elem,num)=>{
                       ...
                       <div>
                        {
                            this.state.xiajiPoliceTypes.length>0?(this.state.xiajiPoliceTypes.map((item,index)=>{
                             
                                    return(
                                        <div className='trace-middle-item trace-item-one' key={index}>
                                            <div data-submitid={item.submitId} onClick={this.showXiajiTasked}  className="data-pointer">
                                                <img src={Police} alt="pic"/>
                                            </div>
                                            <p>{item.deptName?item.deptName.substr(5,):''}</p>
                                        
                                            {/*指派的任务显示*/}
                                            <div className={this.state.assignPoliceTypeId === item.submitId?'showTasked display-block':'display-none'}  onClick={this.stopPropagation.bind(this)}>
                                                <p className='renwuliebiao'>{item.policeTypeName}任务列表</p>
                                            {
                                                this.state.xiajiTaskWillTodo.length>0?(this.state.xiajiTaskWillTodo.map((tasks,index)=>{
                                            
                                                    return(
                                                        <div key={index} className="showTask-item" >
                                                            <div className='showTask-item-time' >{tasks.submitTime?tasks.submitTime:''}</div>
                                                            <div className="showTask-item-desc" >
                                                                <img src={this.state.feedCircle} alt="pic"/>
                                                                {/*<div className="showTask-item-type">
                                                                    <span>{item.policeTypeName} </span>
                                                                </div> */}
                                                                <p>{tasks.taskDesc?tasks.taskDesc:''}</p>
                                                                <button className={(tasks.pushState === '0' || tasks.submitStatus === '0')?"cancleTaskButton":'display-none'} data-submitId={tasks.submitId} onClick={this.deleteZhipaiTask.bind(this)}>撤销</button>
                                                                {
                                                                    tasks.assignFiles.length>0?(tasks.assignFiles.map((elem,num)=>{
                                                                        if(elem.fileType === 'png' || elem.fileType === 'jpeg' ||elem.fileType === 'jpg'  ){
                                                                            return(
                                                                                <a href={elem.filePath} target="_blank" key={elem.fileName}><div className='files-item trace-file-margin'>
                                                                                    <img src={Image} alt="pic"/>
                                                                ...
                                                                {
                                                                    tasks.reportExtList.map((elem,num)=>{
                                                                        return <span className="showReportInTask" onClick={this.showWordModal} data-reportid={elem.reportId}>{elem.reportName}</span>
                                                                    })
                                                                }
