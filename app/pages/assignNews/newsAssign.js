...
import '../assignTopToDown/assignTopToDownLess.less'
...
export default class NewsAssign extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentWillMount() {
        ...
        this.props.dispatch(caseFindById({caseId:this.state.caseId,source:this.state.source}, (res) => {
            if (res.status === 200 && res['data']) {
                this.setState({
                    caseInfo: res.data,
                    // secretLevel:res.data.secretLevel
                },()=>{
                    if(this.state.caseInfo.bssj){
                        this.state.caseInfo.bssj = moment(this.state.caseInfo.bssj).format('YYYY-MM-DD hh:mm:ss')
                    }

                    if(this.state.caseInfo.isLaunch === '0' || this.state.caseInfo.sffq === '0'){
                        this.setState({
                            showAssignBtn: true
                        })
                    }else{
    ...
    //已指派页面返回按钮
    goBack(){
        hashHistory.push(`/master/2/1/${this.state.taskType}`)
    }
    //待审批页
    agreeOrNoOnchange=(e)=> {
        this.setState({
    ...
    showAssign(){
        this.props.dispatch(startCompose({source:this.state.source,caseId:this.state.caseId},(res)=>{
            this.setState({
                showAssign:true,
                id: res.data
            })
    ...
    render() {
        ...
        return (
            <div className="info-content">
                ...
                <div className='inner'>
                    <button className={this.state.showAssignBtn && !this.state.showAssign?"button right":'display-none'} style={{padding:'0.4rem 2rem'}} onClick={this.showAssign.bind(this)}>发起合成</button>
                    ...
                    <div className="inner-table">
                        <table border="1">
                            ...
                            <tr>
                                <td>合成名称</td>
                                <td colSpan={2}>{this.state.caseInfo['caseName']?this.state.caseInfo['caseName']:this.state.caseInfo.bt}</td>
                               {/* <td >密级</td>
                                    <td>{this.state.caseInfo.levelName}</td>*/}
                                <td >类型</td>
                                <td colSpan={2}>{this.state.caseInfo.caseTypeDesc?this.state.caseInfo.caseTypeDesc:(this.state.caseInfo.xxlx==='1'?'综合类情报':this.state.caseInfo.xxlx==='2'?'行动性情报':'')}</td>
                            </tr>
                            <tr>
                                {/*<td>发起人</td>
                                <td >{this.state.caseInfo['username']}</td>*/}
                                <td >来源</td>
                                <td colSpan={2}>{this.state.caseInfo.caseFromDesc?this.state.caseInfo.caseFromDesc:(this.state.caseInfo.cjtj==='1'?'日常工作':this.state.caseInfo.cjtj==='2'?'互联网监察':this.state.caseInfo.cjtj==='3'?'人力情报':this.state.caseInfo.cjtj==='4'?'专案工作':'')}</td>
                                <td>入库时间</td>
                                <td colSpan={2}>{this.state.caseInfo.caseTime?this.state.caseInfo.caseTime:this.state.caseInfo.bssj}</td>
                            </tr>
                            <tr >
                                <td>合成描述</td>
                                <td colSpan="5" dangerouslySetInnerHTML={{__html: this.state.caseInfo['caseDesc']?this.state.caseInfo['caseDesc']:this.state.caseInfo.zw}}>
                    ...
                    <button className={this.state.showAssign?'display-none':"button right"} onClick={this.goBack.bind(this)}>返回</button>
                    <div className={this.state.showAssign?"":"display-none"}>
                        {
