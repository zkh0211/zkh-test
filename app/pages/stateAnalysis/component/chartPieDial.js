...
export default class ChartPieDial extends Component {
    ...
    initOption(newProps) {
        ...
        this.option.title[1].text = newProps.address + '重点人员占比情况';
        var sum = 0;
        if(newProps.data.length > 0){
            for (var i = 0; i < newProps.data.length; i++) {
                sum += newProps.data[i]['SL'];
                this.option.legend.data[i] = newProps.data[i]['ZDRYLB'];
                this.option.series[0].data[i] = {
                    name: newProps.data[i]['ZDRYLB'],
        ...
        this.option.title[0].text = '重点人员总数';
